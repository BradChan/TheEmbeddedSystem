...
的线性关系。其中Data是n个二进制位宽度的数值，Ai是[Amin, Amax]量程范围内的连续信号。很显然，Data的二进制位宽度越大，数字化信号的分辨率越高。
...
由于计算机系统CPU内核和外设使用时钟信号按节奏地工作，且ADC等数字化器件实施信号量化转换的过程需要耗费一定时间(数微秒的量级)，
...
Vout只能输出有限集合{0, RF*I/16, 2*RF*I/16, 3*RF*I/16, .., 15*RF*I/16}中的16个值大小的电压信号。
