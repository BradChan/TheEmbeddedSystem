...
.. Note::  通讯接口相关的基础概念：
  ...
  7. **ModBus通讯协议** ，由Schneider于1979年为PLC开发的一种用于主从架构的异步串行通讯协议，后来成为公认为工业界异步串行通讯协议。该协议的从站资源采用寄存器映射的方法，每个从站有惟一ModBus地址用于寻址，主站可根据从站地址、寄存器地址等信息寻址任一功能寄存器。目前ModBus通讯协议还支持其他类型的物理层网络。
